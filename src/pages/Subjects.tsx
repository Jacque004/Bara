      ...
      <ul className="space-y-2">
        {subjects.length === 0 ? (
          <li>
            <EmptyState
              icon={BookOpen}
              title="Aucune matière encore"
              description="Créez votre première matière ci-dessus — c’est le point de départ de vos tâches et du Focus."
            />
          </li>
        ) : (
          subjects.map((s) => (
            <li
              key={s.id}
              className="flex items-center gap-3 rounded-xl border border-[var(--color-bara-border)] bg-[var(--color-bara-surface)] px-4 py-3 transition hover:border-orange-500/30"
            >
              <span
